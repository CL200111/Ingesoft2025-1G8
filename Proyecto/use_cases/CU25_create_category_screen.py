...
class CreateCategoryScreen(QWidget):
    ...
    def crear_categoria(self):
        ...
        categoria_existente = session.query(Categoria).filter_by(nombre=inserted_nombre).first()
        if categoria_existente:
            self._show_error(f"Ya existe una categoría con el nombre '{inserted_nombre}'.")
            return

        try:
            # Crear categoría
            nueva_categoria = Categoria(
                nombre=inserted_nombre,
            ...
            nueva_categoria = session.query(Categoria).filter_by(nombre=inserted_nombre).first()
            ...
            QMessageBox.information(
                self,
                "✅ Éxito",
                f"Categoría '{inserted_nombre}' creada exitosamente."
            )
            self._clear_form()
