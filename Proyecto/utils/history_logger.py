...
from db.models import Historial, Accion, TargetType

session = Database().get_session()

def write_to_historial(inserted_usuario_id: int, inserted_accion_id: int, inserted_target_type_id: int, inserted_target_id: int):
    """
    ...
    session.commit()
    print(f"✅ Historial registrado: Usuario {inserted_usuario_id} hizo '{inserted_accion_id}' sobre {inserted_target_type_id}({inserted_target_id})")
