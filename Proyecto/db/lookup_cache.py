...
estado_revision = (
    session.query(EstadoLibro).filter_by(nombre="En revisión física").first()
)
estado_revision_aprobada = (
    session.query(EstadoLibro).filter_by(nombre="Aprobado de revisión física").first()
)
estado_restauracion = (
...
estado_digitalizado = (
    session.query(EstadoLibro).filter_by(nombre="Digitalizado").first()
)
estado_calidad = (
    session.query(EstadoLibro).filter_by(nombre="En control de calidad").first()
)
estado_calidad_aprobado = (
    session.query(EstadoLibro)
    .filter_by(nombre="Aprobado por control de calidad")
    .first()
)
estado_clasificacion = (
    session.query(EstadoLibro).filter_by(nombre="En clasificación").first()
)
